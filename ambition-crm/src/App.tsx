...
import { loadData, saveData, resetToDefault, fusionnerDonneesDistantes } from './lib/storage';
import type { CRMData } from './lib/storage';
import { syncWithSupabase, chargerDepuisSupabase } from './lib/sync';
...
export function App() {
  ...
  // sinon des donnees locales perimees pourraient ecraser la base.
  const ignorerProchainEnvoiRef = useRef(true);
  ...
  // Lecture au demarrage : Supabase fait foi, un simple rafraichissement suffit.
  useEffect(() => {
    let annule = false;
    (async () => {
      ...
      if (distant.ok) {
        ...
        setSyncStatus({ message: distant.message, isError: true });
      }
      setChargementDistant(false);
    })();
    return () => {
      annule = true;
    };
        ...
        <div className="flex items-center gap-2">
          {chargementDistant ? (
            <span className="flex items-center gap-1.5 text-blue-600 font-medium">
              <RefreshCw className="w-3 h-3 animate-spin" /> Chargement des données depuis Supabase...
            </span>
